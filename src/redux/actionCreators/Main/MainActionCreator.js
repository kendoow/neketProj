...
export const mainFilter = (news, search) => (dispatch) => {
  if (search === "") {
    ...
    const filterNews = {
      common: news.common.filter((item) =>
        item.tags.toLowerCase().includes(search.toLowerCase())
      ),
      recommended: news.recommended.filter((item) =>
        item.tags.toLowerCase().includes(search.toLowerCase())
      ),
    };
...
export const EditUnpubilshedItem = (updatedPost) => async (dispatch) => {
  try {
    await axios
      .put(`http://localhost:8000/news/unpublished/${updatedPost.id}`, updatedPost)
      .then(({ post }) => {
