...
const GamesInfo = () => {
  const {id, name, title, date, category, 
        description, OS, CPU, RAM, VideoCard, Disk} = useSelector(state => state.games.selectedGame)

  return (
    <div className={styles.Container}>
        <div className={styles.Block}>
            <div className={styles.Title}>{title} - {id}</div>
            <div className={styles.Date}><span>Дата выхода:</span> {date}</div>
            <div className={styles.Category}><span>Жанр:</span> {category}</div>
